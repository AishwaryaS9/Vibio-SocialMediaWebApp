...
const ChatBox = () => {
  ...
  return user && (
    <div className='flex flex-col h-screen' role="main" aria-label="Chat window">
      ...
      <section className="p-5 md:px-10 h-full overflow-y-scroll"
        aria-label="Messages"
        role="log"
        aria-live="polite">
            ...
            .map((message, index) => {
              ...
              return (
                <div key={index} role="article"
                  aria-label={`Message from ${isOwnMessage ? "You" : user.full_name}`}
                  ...
                  >
                    {message.message_type === "image" && (
                      <img
                        ...
                        alt={`Shared by ${isOwnMessage ? "you" : user.full_name}`}
                    ...
                    {isOwnMessage && (
                      <div className="flex justify-end mt-1"
                        ...
                        {message.seen ? (
                          ...
                          <Check size={14} className="text-gray-400" aria-hidden="true" />
                        )}
                      </div>
                    )}
                  </div>
      ...
      <div className="px-4" role="contentinfo">
        <div className="flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5">
          ...
          <label htmlFor="image" aria-label="Upload image">
            {image ? <img src={URL.createObjectURL(image)} alt="Selected image preview"
              className="h-8 rounded" /> :
              <ImageIcon className="size-7 text-gray-400 cursor-pointer" aria-hidden="true" />
            }
