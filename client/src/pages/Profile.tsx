...
import { type FullUser, type Post } from '../utils/helpers';
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden">
          ...
          <div className="h-40 md:h-56 bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-200">
            {user.cover_photo && <img src={user.cover_photo} alt='cover photo'
          ...
          <UserProfileInfo user={user} posts={posts}
            profileId={profileId} setShowEdit={setShowEdit} />
        </div>
          ...
          <div className="bg-white rounded-xl shadow p-1 flex max-w-md mx-auto">
            {["posts", "media", "likes"].map((tab) => (
              ...
              rounded-lg transition-colors cursor-pointer ${activeTab === tab ?
                  "bg-primary text-white" : "text-gray-600 hover:text-gray-900"
                }`}>
                ...
                <>
                  {post.image_urls.map((image, index) => (
                    <Link target='_blank' to={image} key={index} className='relative group'>
                      <img src={image} alt="media" key={index}
